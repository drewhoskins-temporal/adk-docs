...
The `SequentialAgent` is a [workflow agent](https://adk.dev/agents/workflow-agents/index.md) that executes its sub-agents in the order they are specified in the list. Use the `SequentialAgent` when you want the execution to occur in a fixed, strict order.
...
As with other [workflow agents](https://adk.dev/agents/workflow-agents/index.md), the `SequentialAgent` is not powered by an LLM, and is thus deterministic in how it executes. That being said, workflow agents are concerned only with their execution (i.e. in sequence), and not their internal logic; the tools or sub-agents of a workflow agent may or may not utilize LLMs.
